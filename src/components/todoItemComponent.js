...
export class TodoItemComponent extends HTMLElement {
    ...
    connectedCallback() {
        this.contentInput.value = "Click here to enter a task";
        this.contentDisplay.innerHTML = this.contentInput.value;

        TodoListDataService.addTodoItem(this.id, this.contentInput.value, false);

        this.contentDisplay.addEventListener("click", (e) => this.editContent(e));

        this.todoItem.addEventListener("mousedown", (e) => this.todoItemClicked(e));

        document.addEventListener("mousedown", (e) => this.documentClicked(e));

        this.deleteTodoItemBtn.addEventListener("click", (e) => this.deleteTodoItem(e).then(console.log("Success")).catch(function(err) {
            console.log(err);
        }));
    ...
    deleteTodoItem(e) {
        return new Promise((resolve, reject) => {
            ...
            if (todoItem.parentNode && todoItem.parentNode.matches(".todo-items")) {
                resolve(TodoListDataService.deleteTodoItem(todoItem.id), todoItem.parentNode.removeChild(todoItem));
            } else {
